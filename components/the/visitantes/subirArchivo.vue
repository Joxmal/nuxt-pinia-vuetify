<template>
  ...
  <div>
    <v-slide-x-transition group>
      <AlertWarning v-show="store.eliminarExitoso === true" key="1" style="position: fixed; right: 20px; z-index: 20;"
        mensaje="Eliminado con exito" />
      <AlertCargando v-show="store.cargando === true" key="2" style="position: fixed; right: 20px ; z-index: 20; "
        mensaje="Cargando" />
      <AlertSuccess v-show="store.editarExitoso === true" key="3" style="position: fixed; right: 20px; z-index: 20;"
        mensaje="Editado" icon="mdi-pencil-outline" />
      <AlertError v-show="store.ocurrioUnError === true" key="4" style="position: fixed; right: 20px; z-index: 20;"
        mensaje="ERROR" />
      <AlertSuccess v-show="store.envioExitoso === true" key="5" style="position: fixed; right: 20px; z-index: 20;"
  ...
  <v-container class="bg-surface h-screen">
    <v-row class="my-2">
      <v-col cols="12" md="6">
        <v-text-field v-model="store.form.departamento" hide-details="auto" label="Departamento" />
      </v-col>

      <v-col cols="12" md="6">
        <v-text-field v-model="store.form.razon" hide-details="auto" label="Razón" />
      </v-col>
    ...
    <v-card position="relative" class="d-flex justify-center align-center" style="overflow: auto;" border="colorLink lg"
      height="250" @dragover="dragover" @dragleave="dragleave" @drop="drop">
      <input style="display: none;" type="file" multiple name="file" id="fileInput" class="hidden-input"
      ...
      <div class="align-content-start d-flex flex-wrap ga-2 justify-center " v-if="files.length">
        <v-card class="" elevation="10" width="90px" height="90px" position="relative" border="success md"
          ...
          <div>
            <v-btn color="primary" location="top right" position="absolute" density="compact" class="rounded-circle"
              type="button" @click="remove(files.indexOf(file))" title="Remove file" icon="mdi-delete-forever">
            </v-btn>
          </div>
        </v-card>
      </div>
    ...
    <v-row align="center" justify="center" class="">
      <v-col cols="4" md="3" lg="2">
        <v-card :color="tamanoFiles >= 20 ? 'error' : 'success'" elevation="10" class=" text-center">{{ tamanoFiles +
          "MB"
          }}</v-card>
      </v-col>
      <v-divider class="border-opacity-0"></v-divider>
      <v-col cols="10">
        <v-btn :loading="loadinButton"
          :disabled="tamanoFiles >= 20 || files.length === 0 || (store.form.departamento.length === 0 || store.form.razon.length === 0) || loadinButton === true"
          color="primary" width="100%" @click="enviarArchivos">
          Enviar
        </v-btn>
      </v-col>
    </v-row>
...
const enviarArchivos = async () => {
  ...
  if (totalSize > 509715200) {
    alert("No puede enviar mas de 500MB de en peso de archivos")
    return
  }
