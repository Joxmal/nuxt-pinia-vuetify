<template>
  <pre>
    {{storeActividades.BD_procesada_listaAtividades}}
  </pre>
  ...
  />
  <pre>
    {{console.log(storeActividades.BD_procesada_listaAtividades) }}

    <!-- {{ storeActividades.BD_listaAtividades }} -->
  </pre>
  ...
  <v-card class="mx-auto"  rounded="lg">
    ...
    <div class="pa-4">
      <TableGeneral v-if="(Object.keys(storeActividades.BD_procesada_listaAtividades).length > 0)"
      titulo_table="Actividades"
      :titulos="titulos"
      :listaItems="storeActividades.itemsTabla_BD_procesada_listaAtividades"
      :loading="(Object.keys(storeActividades.BD_procesada_listaAtividades).length > 0 ? true : false)"
...
const titulos = [
  ...
  { key: 'description', title: 'Descripsión',align:'center',sortable: false,},
...
onMounted(async()=>{
 await storeActividades.obtenerActvidades()
  console.log(storeActividades.buscarActividadIdPorNombre("mantenimiento preventivo servidor"))
  console.log(storeActividades.buscarActividadIdPorNombre("ASIST. TÉCNICA"))
})

watch(() => storeActividades.count_reload , async (newValue) => {
...
watch(() => modoEditar.value , async (newValue) => {
  modoEditar.value === false ?  titulo_dialog.value = 'Crear' : titulo_dialog.value = 'Editar'
