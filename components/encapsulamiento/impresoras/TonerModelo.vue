...
<template>
  ...
  <v-card height="650px" color="background" elevation="10" border position="relative"   
    ...
    <v-slide-y-transition group>
      ...
      <CardImage   :color="tooner.bgColor" width="280"  height="230" v-for="(tooner,index) in storeToonerModelo.itemsToonerModelo" :key="index"
      @click="$emit('seleccionModelo',tooner),CambiarColorCardModelo(index)"
  ...
  const emits = defineEmits(['seleccionModelo'])
  ...
  const selectedColor = ref('surface')

  function CambiarColorCardModelo(index) {
    // Reiniciar el color de todas las tarjetas
    storeToonerModelo.itemsToonerModelo.forEach(element => {
      element.bgColor= "surface"
    });
    // Establecer el color de la tarjeta seleccionada
    storeToonerModelo.itemsToonerModelo[index].bgColor = 'success'
  }
  ...
  onMounted(async()=>{
    await storeToonerModelo.ObtenerModeloTooner()
