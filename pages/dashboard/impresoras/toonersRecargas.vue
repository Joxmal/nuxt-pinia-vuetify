<template>
  <div>
    <v-slide-x-transition group>
      <AlertWarning v-show="storeToonersRecargas.eliminarExitoso === true" key="1"
        style="position: fixed; right: 20px; z-index: 20;" mensaje="Eliminado con exito" />
      <AlertCargando v-show="storeToonersRecargas.cargando === true" key="2"
        style="position: fixed; right: 20px ; z-index: 20; " mensaje="Cargando" />
      <AlertSuccess v-show="storeToonersRecargas.editarExitoso === true" key="3"
        style="position: fixed; right: 20px; z-index: 20;" mensaje="Editado" icon="mdi-pencil-outline" />
  ...
  <DialogGeneralSimple id-boton="MostrarDetallesTonerRecargas" name-boton="adds" :ocultar-boton="true">
    <template #contenido>
      <div style="width: 100%; margin: 0 auto; ">
        <v-card max-height="90vh" style="overflow: auto;" position="relative" elevation="15"
          ...
          <v-container>
            ...
            <v-row class="">
              <v-col cols="12" sm="4" v-for="(valor, clave) in storeToonersRecargas.descripsionTonerRecarga"
                :key="clave">
              ...
              <v-slide-x-transition group>
                <v-col cols="3" v-for="(valor, clave) in storeToonersRecargas.descripsionTonerListaRecarga"
                  :key="clave">
                  <v-card position="relative">
                    ...
                    <v-card-text class="text-center">
                      {{ new Date(valor.recarga).toLocaleDateString('es-ES', {
                        weekday: 'long', year: 'numeric', month:
                          'long', day: 'numeric'
                      }) }}
                    </v-card-text>
                    <v-btn @click="storeToonersRecargas.restarTonerRegarca2({ toner: valor })" density="compact"
                      position="absolute" location="top right" icon="mdi-delete-forever"></v-btn>
                  </v-card>
  ...
  <DialogForm @crear="storeToonersRecargas.EnviartonerRecarga()" id_boton="creacionModeloTooner"
    :titulo_dialog="tituloDialogToonerRecarga" boton_titulo="Nuevo Toner" @cerrar="modoEditar = false"
    :modo-editar="modoEditar" @editar-dialog-form="storeToonersRecargas.editarTonerRecarga({ ID: IdEditar })">
    <template #contenido>
          ...
          <DialogGeneralSimple id-boton="MostrarModelos" :ocultar-boton="false" name-boton="Modelo Tooner">
            <template #contenido>
              <div style="width: 80%; margin: 0 auto; ">
                <v-card max-height="90vh" style="overflow: auto;" position="relative" elevation="15"
                  class="d-flex flex-column ga-2">
                  <EncapsulamientoImpresorasTonerModelo modo-seleccion @seleccion-modelo="capturarTonerModelo" />
                </v-card>
              </div>
            </template>
          </DialogGeneralSimple>

          <CardImage class="mx-auto mt-2" color="surface" width="280" height="120" src-image="/images/toonerImage.png "
      ...
      <v-container>
        <v-row>
          <v-col cols="12" md="6">
            <v-text-field label="Nro Item" maxlength="30" counter :rules="ruleNoEmpty" hint="numero unico del tooner"
              persistent-hint v-model="storeToonersRecargas.formToonerRecarga.nro_item">
            </v-text-field>
          </v-col>

          <v-col cols="12" md="6" class="m-auto">
            <v-text-field type="number" label="Recargas" min="0" max="100"
              hint="Si esta nuevo colocar (0), si ya llego usado del exterior coloque su Nro" persistent-hint
          ...
          <v-col cols="12">
            <v-autocomplete label="departamento" :items="useListasStore().listaDepartamento"
              v-model="storeToonersRecargas.formToonerRecarga.direccion" :rules="ruleNoEmpty">

            </v-autocomplete>
          </v-col>

          <v-col cols="12">
            <v-textarea counter label="Descripsion" hint="Descripsion del Modelo de Tooner (opcional)" maxlength="400"
              persistent-hint v-model="storeToonersRecargas.formToonerRecarga.descripsion">
            </v-textarea>
          </v-col>
        </v-row>
      </v-container>
  ...
  <v-form @submit.prevent>
    <v-row align="center" align-content="center" justify="center">
      <v-col style="border: 2px solid rgb(54, 177, 218);border-radius: 30px;" cols="8" sm="3"
        ...
        <div class="d-flex justify-center align-center ">
          <v-checkbox v-model="FiltrotonerRecarga.rangoFechas.activo" :density="'compact'"
            :hide-details="'auto'" />Fecha
          de entrada
        </div>
        <div v-if="FiltrotonerRecarga.rangoFechas.activo">
          <div>
            ...
            <input class="" type="date" min="2023-01-01" max="2024-12-30"
              v-model="FiltrotonerRecarga.rangoFechas.desde">
          </div>
          ...
          <div>
            HASTA
            <input class="" type="date" min="2023-01-01" max="2024-12-30"
              v-model="FiltrotonerRecarga.rangoFechas.hasta">
          </div>
    ...
    <v-row align="center" align-content="center" justify="center" class="mt-2">
      <v-col cols="12" md="2">
        <v-text-field density="compact" hide-details="auto" label="nro Item"
          v-model="storeToonersRecargas.FiltrotonerRecarga.nro_item" />
      </v-col>
      <v-col cols="12" md="2">
        <v-text-field density="compact" hide-details="auto" label="Nro Recargas" type="number" min="0"
          v-model="storeToonersRecargas.FiltrotonerRecarga.nro_regargas" />
      </v-col>

      <v-col cols="12" md="2">
        <v-autocomplete :items="storeToonersModelos.FiltrobusquedaMarcaModelo_autocomplete.marca" label="Marca"
          hide-details="auto" density="compact" v-model="storeToonersRecargas.FiltrotonerRecarga.marca">

        </v-autocomplete>
      </v-col>

      <v-col cols="12" md="2">
        <v-autocomplete :items="storeToonersModelos.FiltrobusquedaMarcaModelo_autocomplete.modelo" label="Modelo"
          hide-details="auto" density="compact" v-model="storeToonersRecargas.FiltrotonerRecarga.modelo">
        </v-autocomplete>
      </v-col>

      <v-col cols="12" md="2">
        <v-autocomplete :items="useListasStore().listaDepartamento" label="Departamento" hide-details="auto"
          density="compact" v-model="storeToonersRecargas.FiltrotonerRecarga.departamento">
        </v-autocomplete>
      </v-col>

      <v-col cols="12" md="1">
        <v-btn @click="storeToonersRecargas.obtenerToonersRecargas()" width="100%" type="submit"
          class="px-2">Buscar</v-btn>
      </v-col>
    </v-row>

    <v-row align="center" align-content="center" justify="center">
      <div>
        <v-switch v-model="storeToonersRecargas.FiltrotonerRecarga.tonerInactivo" inset density="compact"
          base-color="primary"
          :color="storeToonersRecargas.FiltrotonerRecarga.tonerInactivo === false ? 'secondary' : 'red'" hide-details
          append-icon="mdi-cog-off" prepend-icon="mdi-cog">
        </v-switch>
      </div>
    </v-row>
  </v-form>
  ...
  <div class="d-flex justify-center mt-10">
    <v-card elevation="10" color="primary" class="pa-2 rounded-lg text-center font-weight-bold  text-h6">{{
      storeToonersRecargas.toonerModeloRecarga.totalItems }}</v-card>
  </div>
  <v-card height="650px" color="background" elevation="10" border position="relative"
    ...
    <v-slide-y-transition group>
      <CardImage :color="tonerRecarga.activo ? 'surface' : 'red-lighten-2'" width="350" height="230"
        v-for="(tonerRecarga, index) in storeToonersRecargas.itemsToonerRecarga" :key="index"
        src-image="/images/toonerImage.png " ocultar-boton-seleccion :title="`${tonerRecarga.direccion}`"
        ...
        button-name="">
        <template #menu>
          <MenuDropdown @eliminar="storeToonersRecargas.eliminarTonerRecarga({ ID: tonerRecarga.id })"
            @editar="editarDescripsion({ toner: tonerRecarga })"
            @descripcion="abrirDialogoDescripsion({ toner: tonerRecarga })" opcion-extra="Activar-Desactivar"
          ...
          <v-btn size="large"
            @click="storeToonersRecargas.sumarTonerRegarca({ ID: tonerRecarga.id, nroActual: tonerRecarga.nro_regargas })"
            density="comfortable" icon="mdi-plus" color="success" position="absolute"
            style="bottom: 5px; right: 5px;"></v-btn>
          <v-sheet border :class="tonerRecarga.nro_regargas > 5 ? 'text-warning' : 'text-success'"
            class="text-h4 px-2 rounded-lg" density="compact" color="surface" elevation="20" position="absolute"
            location="top">
  ...
  <div class="d-flex justify-space-between align-center pa-2" style="position: relative;">
    <v-pagination class="mx-auto" :density="'compact'" v-model="storeToonersRecargas.toonerModeloRecarga.page"
      :total-visible="3" :length="storeToonersRecargas.toonerModeloRecarga.totalPages">
    </v-pagination>
    <div v-show="name !== 'xs'" style="position: absolute; right: 0; top: 50%;transform: translate(0%, -25%);">
      <v-select density="compact" :items="[10, 20, 30, 40, 50, 300, 10000]"
...
function onDetect(detectedCodes) {
  qrCode.value = detectedCodes[0]; // Get the first detected QR code
}
