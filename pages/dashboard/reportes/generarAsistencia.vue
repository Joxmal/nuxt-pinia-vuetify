<template>
        ...
        <v-row>
          ...
          <v-col cols="12">
            ...
            <v-card elevation="15" class="mx-auto" max-width="500px"
              ...
              <label :class="store.form.status === false ? 'text-red' : 'text-success'" class="text-h5">{{
                store.form.status == true ? 'FINALIZADO' : 'EN CURSO' }}</label>
            </v-card>
          </v-col>
        </v-row>
      </v-container>
  ...
  <v-card height="70vh" position="relative"
    ...
    <div v-else class="d-flex flex-wrap justify-center ga-2">
      ...
      <lazy-card-asistencia v-for="item in store.asistenciaLista_Usuario.items " :key="item.id" class="border"
  ...
  <v-select v-show="name === 'xs'" density="compact" :items="[10, 20, 30, 40, 50, 300]" v-model="paginationItemsPorPagina" />
...
onMounted(() => {
  ...
  console.log(new Date(store.obtenerFechaHoraActual_2pm))
})
