...
export const useListasActividadesStore = defineStore('useListasActividadesStore', {
  state: () => ({
    //

    // lista de las areas de informatica
    listaAreas: ['Sistemas','Técnico'],
  ...
  actions: {
    ...
    async obtenerActvidades() {
      ...
      try {
        const records = await pb.collection('Tareas_areas').getFullList({
          sort: '-created',
          expand: `area`
        });
        ...
        this.BD_listaAtividades.forEach((tarea, index) => {
          ...
          const tareaDescription = tarea.description || ''; // Obtener la descripción de la tarea
          const tareaNombre = tarea.tarea
    ...
    async eliminarActividad({id}){
      ...
      try {
        await pb.collection('Tareas_areas').delete(id);
